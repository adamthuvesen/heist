...
def test_capture_usage_sums_per_turn_costs_across_events() -> None:
    # Per-turn cost shapes (bare cost_usd here) must SUM across lines, matching
    # how tokens are summed — taking the max would under-count an N-turn session.
    stream = "\n".join('{"type":"message_delta","cost_usd":0.10}' for _ in range(4))

    capture = capture_usage(stream)

    assert round(capture.reported_cost_usd or 0.0, 4) == 0.40
    assert capture.reported_cost_source == "cost_usd"


def test_capture_usage_sums_per_turn_model_usage_costs_across_events() -> None:
    # modelUsage emitted once per turn — the per-turn aggregates sum across lines.
    stream = "\n".join(
        '{"type":"step","modelUsage":{"claude-opus-4-8":{"costUSD":0.25}}}' for _ in range(3)
    )

    capture = capture_usage(stream)

    assert round(capture.reported_cost_usd or 0.0, 4) == 0.75


def test_capture_usage_prefers_session_total_over_per_turn_sum() -> None:
    # A cumulative total on the final line is authoritative; it is NOT summed
    # with the per-turn modelUsage lines that precede it.
    stream = (
        '{"type":"step","modelUsage":{"m":{"costUSD":0.25}}}\n'
        '{"type":"step","modelUsage":{"m":{"costUSD":0.25}}}\n'
        '{"type":"result","total_cost_usd":0.50}'
    )

    capture = capture_usage(stream)

    assert capture.reported_cost_usd == 0.50
    assert capture.reported_cost_source == "total_cost_usd"


def test_capture_usage_does_not_double_count_model_usage_as_bare_cost() -> None:
    # costUSD lives inside modelUsage AND matches the bare-cost key set. It must
    # be counted once (the tier-1 aggregate), not also as a tier-2 bare cost.
    capture = capture_usage('{"type":"result","modelUsage":{"only":{"costUSD":1.00}}}')

    assert capture.reported_cost_usd == 1.00
