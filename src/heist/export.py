...
def _eval_audit_row(result: TaskRunResult) -> dict[str, object]:
    return {
        ...
        "partial_credit": result.partial_credit if result.outcome_status == "graded" else None,
...
def export_eval_audit(run_dir: Path, results: list[TaskRunResult]) -> Path:
    ...
    rows = [_eval_audit_row(result) for result in results]
